//! test_caller_b — integration test fixture. Thin pass-through to
//! keystore_signer using this module's own credential (managed by
//! keystore-signer-client), so an external test driver (see
//! `tests/flake.nix`'s `isolation-test` check) can prove that test_caller_a
    ...
    fn secret_hex(&mut self) -> String {
        if self.credential.is_none() {
            let path = context()
                ...
                .filter(|p| !p.is_empty())
                .expect("test_caller_b requires a host-provisioned instance_persistence_path");
            self.credential =
